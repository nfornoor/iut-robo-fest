...
export default function OfficeLayout({ devicesByRoom }) {
  return (
    <div className="office-layout-card">
      ...
      >
        {/* Outer shell */}
        <rect x="10" y="10" width="880" height="380" rx="14" className="office-shell" />
          ...
          return (
            <g key={room.id} transform={`translate(${bounds.x}, 20)`}>
              <rect
                ...
                className="room-rect"
              />
              <text x={bounds.w / 2} y="26" textAnchor="middle" className="room-label">
              ...
              {room.id === "drawing" ? (
                <>
                  <rect x="30" y="230" width="90" height="45" rx="8" className="furniture-sofa" />
                  <rect x="140" y="240" width="60" height="30" rx="4" className="furniture-table" />
                </>
              ) : (
                <>
                  <rect x="40" y="200" width="60" height="34" rx="4" className="furniture-desk" />
                  <rect x="180" y="200" width="60" height="34" rx="4" className="furniture-desk" />
                </>
              )}

              {fans.map((device, i) => (
                <g key={device.id} transform={`translate(${ANCHORS.fan[i].x}, ${ANCHORS.fan[i].y})`}>
                  <FanIcon device={device} />
                </g>
              ))}
              {lights.map((device, i) => (
                <g key={device.id} transform={`translate(${ANCHORS.light[i].x}, ${ANCHORS.light[i].y})`}>
